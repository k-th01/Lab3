...
@app.get("/user_posts/{user_id}")
def fetch_user_posts_with_comments(user_id: int):
    
    user_posts_response = requests.get(f'https://jsonplaceholder.typicode.com/posts?userId={user_id}')
    user_posts = json.loads(user_posts_response.text)
    
    response_data = {"user_id": user_id, "posts_with_comments": []}
    
    for post in user_posts:
        
        post_comments_response = requests.get(f'https://jsonplaceholder.typicode.com/comments?postId={post["id"]}')
        post_comments = json.loads(post_comments_response.text)
        
        response_data["posts_with_comments"].append({
            "post_id": post["id"],
            "title": post["title"],
            "body": post["body"],
            "comments": [
